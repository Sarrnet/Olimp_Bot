...
export class TrainingGenerator {
    async getDailyPlan(
        ...
        // 2. Fetch last session to avoid same exercises tomorrow
        const lastSession = await prisma.trainingSession.findFirst({
            where: { userId },
            orderBy: { date: 'desc' },
        })
        const lastExercises = lastSession ? (lastSession.exercises as string[]) : []

        // 3. Fetch all previous sessions to prioritize new exercises
        const allPreviousSessions = await prisma.trainingSession.findMany({
            where: { userId },
            select: { exercises: true },
        })
        const seenExercises = new Set<string>()
        allPreviousSessions.forEach((s) => {
            ;(s.exercises as string[]).forEach((id) => seenExercises.add(id))
        })
        ...
        const multiplier =
            ...
            rule.duration_multiplier_range[0] + longTermBonus
