...
import { handleTributeChatMember } from './bot/handlers/tribute.handler.js'
...
// Tribute chat_member handler
bot.on('chat_member', (ctx) => handleTributeChatMember(ctx))
...
async function triggerInvoiceMenu(ctx: MyContext, days: number = 30) {
    ...
    const buttons = []
    const providers = Object.values(PAYMENT_PROVIDERS).filter(
        (p) => p.id === 'telegram_stars' || !!p.token,
    )
            ...
            const starsPrice =
                days === 180
                    ...
                    : days === 90
                      ? config.price3Stars
                      : config.priceStars
            label1 = `${p1.flag} ${i18n.t(lang, 'messages.stars_amount_label', { amount: starsPrice })}`
        } else if (p1.id === 'crypto_pay') {
            label1 = `${p1.flag} ${p1.label} (USDT / TON / BTC)`
        } else if (p1.id === 'tribute') {
            label1 = `${p1.flag} ${p1.label} (Tribute)`
        } else {
            const amount1 = await currencyService.convertFromRUB(priceRUB, p1.currency)
            label1 = `${p1.flag} ${p1.label} (${amount1} ${p1.currency})`
        }
        row.push({
            ...Markup.button.callback(label1, `pay:provider:${p1.id}:${days}`),
            style: 'success',
        } as any)
                ...
                const starsPrice =
                    days === 180
                        ...
                        : days === 90
                          ? config.price3Stars
                          : config.priceStars
                label2 = `${p2.flag} ${i18n.t(lang, 'messages.stars_amount_label', { amount: starsPrice })}`
            } else if (p2.id === 'crypto_pay') {
                label2 = `${p2.flag} ${p2.label} (USDT / TON / BTC)`
            } else if (p2.id === 'tribute') {
                label2 = `${p2.flag} ${p2.label} (Tribute)`
            } else {
                const amount2 = await currencyService.convertFromRUB(priceRUB, p2.currency)
                label2 = `${p2.flag} ${p2.label} (${amount2} ${p2.currency})`
            }
            row.push({
                ...Markup.button.callback(label2, `pay:provider:${p2.id}:${days}`),
                style: 'success',
            } as any)
        }
        buttons.push(row)
    ...
    buttons.push([
        {
            ...Markup.button.callback(
                `🧑‍💻 ${i18n.t(lang, 'buttons.manual_payment')}`,
                `pay:manual:${days}`,
            ),
            style: 'success',
        } as any,
...
bot.action(/^pay:provider:(.+):(\d+)$/, async (ctx) => {
    ...
    try {
        if (providerId === 'crypto_pay') {
            const amountUSDT = await currencyService.convertFromRUB(priceRUB, 'USDT')
        ...
        if (providerId === 'tribute') {
            const tributeUrl =
                process.env.TRIBUTE_PAYMENT_URL || 'https://t.me/tribute/app?startapp=...'
            return ctx.reply(i18n.t(ctx.language || 'ru', 'messages.tribute_payment_info'), {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'Pay with Tribute', url: tributeUrl, style: 'success' } as any],
                    ],
                },
            })
        }
