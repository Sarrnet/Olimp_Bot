...
import { setupDefaultCommands } from './bot/commands.js'
...
import {
    ...
    handleAdminABDelete,
    handleAdminABAskPrice,
    handleAdminABAskNewGroup,
    handleAdminMessage,
    isAdmin,
} from './bot/handlers/admin.handler.js'

import { handleWater, handleSleep, handleWeight, processHealthInput } from './bot/handlers/health.handler.js'
...
async function triggerInvoiceMenu(ctx: MyContext, days: number = 30) {
    const config = ctx.abConfig || { price: 699, price3: 1799, price6: 2999 }
    const lang = ctx.language || 'ru'
    let priceRUB = config.price
    let title = i18n.t(lang, 'messages.tariff_1m')
    
    if (days === 90) {
        priceRUB = config.price3
        title = i18n.t(lang, 'messages.tariff_3m')
    } else if (days === 180) {
        priceRUB = config.price6
        title = i18n.t(lang, 'messages.tariff_6m')
    }

    const buttons = []
    const providers = Object.values(PAYMENT_PROVIDERS).filter(p => !!p.token)

    for (let i = 0; i < providers.length; i += 2) {
        const row = []
        const p1 = providers[i]
        const amount1 = await currencyService.convertFromRUB(priceRUB, p1.currency)
        const label1 = `${p1.flag} ${p1.label} (${amount1} ${p1.currency})`
        row.push(Markup.button.callback(label1, `pay:provider:${p1.id}:${days}`))

        if (providers[i + 1]) {
            const p2 = providers[i + 1]
            const amount2 = await currencyService.convertFromRUB(priceRUB, p2.currency)
            const label2 = `${p2.flag} ${p2.label} (${amount2} ${p2.currency})`
            row.push(Markup.button.callback(label2, `pay:provider:${p2.id}:${days}`))
        }
        buttons.push(row)
    }
    ...
    await ctx.reply(
        i18n.t(lang, 'messages.payment_choice_title', { title }),
...
bot.command('buy', (ctx) => {
    const lang = ctx.language || 'ru'
    const config = ctx.abConfig || { price: 699, price3: 1799, price6: 2999, oldPrice: 899, oldPrice3: 2697, oldPrice6: 5394 }
    return ctx.reply(formatTariffs(config, lang), {
        parse_mode: 'HTML',
        ...Markup.inlineKeyboard([
            [Markup.button.callback(`💳 ${i18n.t(lang, 'messages.tariff_1m')}`, 'pay:1m'), Markup.button.callback(`🔥 ${i18n.t(lang, 'messages.tariff_3m')}`, 'pay:3m')],
...
bot.action(/^pay:provider:(.+):(\d+)$/, async (ctx) => {
    ...
    try {
        ...
        const minorUnits = Math.round(amountConverted * 100)
...
async function handleProfile(ctx: MyContext) {
    ...
    try {
        ...
        const lastWeight = Object.values(weightLogs).pop() || (user.onboardingData as any)?.weight_kg || '-'

        const profileMsg = i18n.t(lang, 'messages.profile_stats', {
            currentHeight: user.currentHeight || (user.onboardingData as any)?.current_height_cm || '-',
            weight: lastWeight,
            bmi: user.initialBmi || '-',
...
bot.action(/^admin:ab:price:(.+)$/, async (ctx) => {
    await handleAdminABAskPrice(ctx, ctx.match[1])
})
...
bot.on('successful_payment', async (ctx) => {
    ...
    try {
        ...
        logger.info(`Successful payment from user ${telegramId} for ${days} days via ${providerId}`)
        await ctx.reply(`✅ ${i18n.t(lang, 'messages.payment_success')}\n\n🏔️ Expiry: <b>${newExpiry.toLocaleDateString(lang === 'ru' ? 'ru-RU' : 'en-US')}</b>`, { parse_mode: 'HTML' })
    } catch (error) {
...
bot.catch(async (err, ctx) => {
    ...
    const lang = ctx.language || 'ru'
    await ctx.reply(i18n.t(lang as 'ru', 'messages.global_error')).catch(() => {})
})
...
process.on('unhandledRejection', (reason, promise) => logger.error('CRITICAL: Unhandled Rejection at:', promise, 'reason:', reason))
