...
// Register payment handlers (Boosty, etc.)
...
async function triggerInvoiceMenu(ctx: MyContext, days: number = 30) {
    ...
    const otherProviders = Object.values(PAYMENT_PROVIDERS).filter((p) => {
        ...
        return p.id !== 'telegram_stars' && p.id !== 'crypto_pay' && p.id !== 'tribute' && !!p.token
    })
...
async function sendAnalysisFlow(ctx: MyContext, analysisData: any, isPaid: boolean) {
    const config = ctx.abConfig || {
        price: 699,
        price3: 1799,
        price6: 2999,
    }
    const analysisMessages = formatAnalysisForUser(analysisData, isPaid, ctx.language, config)

    // 1. Отправляем каждый блок отдельным сообщением
    for (const message of analysisMessages) {
        await ctx.reply(message, { parse_mode: 'HTML' })
    }

    // 2. Отправляем Upsell картинку с текстом (только если не оплачено)
    if (!isPaid) {
        const upsellCaption = `🏔 Хватит копировать чужие общие советы по увеличению роста из интернета - они не адаптированы под тебя и поэтому <b>бесполезны</b>.
      ...
      ⏳ <b>Хватит тратить свое драгоценное время</b>. С каждым днем бездействия <b>твои зоны роста закрываются все сильнее</b>, а шансы стать выше - просто тают. Активируй свой <b>личный план Увеличения Роста</b>, выбрав тариф ниже:`
        ...
        // 3. Вызов меню тарифов
        await triggerInvoiceMenu(ctx)
    }
}
    ...
    try {
        ...
        if (user && user.paid) {
            ...
            await bot.telegram.sendMessage(
                userId,
                '❌ Для вступления необходимо оплатить подписку в боте.',
            )
        }
    } catch (error) {
...
bot.launch({ dropPendingUpdates: true, allowedUpdates: ['message', 'callback_query', 'chat_member', 'my_chat_member', 'chat_join_request', 'pre_checkout_query'] })
    .then(() => {
