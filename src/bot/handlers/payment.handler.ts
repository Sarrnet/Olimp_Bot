...
export function registerPaymentHandlers(bot: Telegraf<MyContext>) {
    ...
    // 1. Обработчик нажатия на кнопку Boosty в меню тарифов
        ...
        try {
            ...
            await ctx.reply(message, {
                ...
                ...Markup.inlineKeyboard([
                    [Markup.button.url('Перейти на Boosty', boostyUrl)]
                ])
            });
        } catch (error) {
            logger.error('Error in pay:boosty action:', error);
            await ctx.reply('Извините, произошла ошибка при подготовке ссылки на Boosty.');
        }
    });

    // 2. Обработчик события chat_member для автоматической активации/деактивации
