...
export async function handleGetAnalysis(ctx: MyContext) {
    ...
    try {
        ...
        // 1. Отправляем Visual Board отдельным сообщением без текста (чтобы избежать Caption Too Long)
        if (imageToCache) {
            const photoInput =
            ...
            try {
                ...
                logger.error('Error sending visual board photo:', err)
            }
        ...
        // 2. Обработка и отправка текста анализа чанками
        // Поиск и извлечение блока "Ваш потенциальный генетический рост"
        const potentialGrowthIndex = analysisMessages.findIndex((m) =>
            m.includes('Ваш потенциальный генетический рост') ||
            m.includes('Your potential genetic height')
        )
        let potentialGrowthMsg = ''
        if (potentialGrowthIndex !== -1) {
            potentialGrowthMsg = analysisMessages.splice(potentialGrowthIndex, 1)[0]
        }

        // Разбиваем на чанки по 3 пункта для стабильности отображения
        const CHUNK_SIZE = 3
        ...
        // Отправляем блок с потенциалом роста отдельным самостоятельным сообщением
        if (potentialGrowthMsg) {
            await ctx.reply(potentialGrowthMsg, { parse_mode: 'HTML' })
        }
