...
export async function handleGetAnalysis(ctx: MyContext) {
    ...
    try {
        ...
        // We split the first chunk at 1000 to be safe, then the rest at 4000.
        const firstChunk = formattedMessage.substring(0, 1000)
        const lastNewLine = firstChunk.lastIndexOf('\n')
        const splitIndex = lastNewLine > 500 ? lastNewLine : 1000
        
        const firstPart = formattedMessage.substring(0, splitIndex)
        const remainingPart = formattedMessage.substring(splitIndex)
        
        const otherChunks = remainingPart ? splitMessage(remainingPart, 4000) : []
        const chunks = [firstPart, ...otherChunks]
