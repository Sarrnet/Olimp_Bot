    ...
    try {
        ...
        if (!user.paid) {
            ...
            keyboard.push([
                {
                    ...Markup.button.callback(
                        `💳 1 мес — ${config.price}₽ (-${calc(config.oldPrice, config.price)}%)`,
                        'pay:1m',
                    ),
                    style: 'success',
                } as any,
            ...
            keyboard.push([
                {
                    ...Markup.button.callback(
                        `🔥 3 мес — ${config.price3}₽ (-${calc(config.oldPrice3, config.price3)}%)`,
                        'pay:3m',
                    ),
                    style: 'success',
                } as any,
            ...
            keyboard.push([
                {
                    ...Markup.button.callback(
                        `💎 6 мес — ${config.price6}₽ (-${calc(config.oldPrice6, config.price6)}%)`,
                        'pay:6m',
                    ),
                    style: 'success',
                } as any,
        ...
        if (imageToCache) {
            ...
            try {
                await ctx.replyWithHTML(
                    chunks[0],
                    chunks.length === 1 ? Markup.inlineKeyboard(keyboard) : undefined,
                )
            } catch (err: any) {
                if (err.description && err.description.includes("can't parse entities")) {
                    await ctx.reply(
