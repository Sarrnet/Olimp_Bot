...
export async function handleGetAnalysis(ctx: MyContext) {
    ...
    try {
        ...
        // 1. Отправляем первый блок с картинкой (Visual Board)
        if (imageToCache) {
            const photoInput =
            ...
            await ctx.reply(analysisMessages[0], { parse_mode: 'HTML' })
        }

        // 2. Отправляем остальные блоки по одному
        for (let i = 1; i < analysisMessages.length; i++) {
            await new Promise((resolve) => setTimeout(resolve, 200))
            await ctx.reply(analysisMessages[i], { parse_mode: 'HTML' })
        }
        ...
        if (!user.paid) {
          ...
          ⏳ <b>Хватит тратить свое драгоценное время</b>. С каждым днем бездействия <b>твои зоны роста закрываются все сильнее</b>, а шансы стать выше - просто тают. Активируй свой <b>личный план Увеличения Роста</b>, выбрав тариф ниже:`;
            ...
            await ctx.replyWithPhoto(
                { source: './assets/images/logo.jpg' }, // Use a valid placeholder or the requested IMG_3856.PNG if exists
                {
