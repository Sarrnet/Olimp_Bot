...
import {
    ...
    OnboardingStep,
    loadOnboardingData,
} from '../../utils/onboarding-parser.js'
...
const handleAnswer = async (ctx: MyContext, step: OnboardingStep, lang: string) => {
    ...
    if (ctx.callbackQuery && 'data' in ctx.callbackQuery) {
        ...
        if (text === '/cancel') {
            await ctx.reply(i18n.t(lang, 'messages.onboarding_cancelled'), Markup.removeKeyboard())
        ...
        if (!validation.isValid) {
            await ctx.reply(
                `⛰️ ${validation.errorMessage || i18n.t(lang, 'messages.invalid_input')}`,
            )
            return false
        }
...
export const onboardingScene = new Scenes.WizardScene<MyContext>(ONBOARDING_SCENE_ID, ...handlers)
