...
import { getOnboardingSteps, OnboardingStep } from '../../utils/onboarding-parser.js'
...
const createStepHandler = (index: number) => {
    return async (ctx: MyContext) => {
        ...
        const onboardingData = await import('../../utils/onboarding-parser.js').then(m => m.loadOnboardingData(lang));
        ...
        if (success) {
            if (index < steps.length) {
                ...
                await ctx.reply(
                    `🏔️ ${i18n.t(lang, 'messages.onboarding_finished')}${bmi ? i18n.t(lang, 'messages.bmi_result', { bmi }) : ''}`,
